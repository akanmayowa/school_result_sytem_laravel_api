...
class CalculateResultOne implements ShouldQueue
{
    ...
    {
        $candidates = ScoreMarkerOne::whereNew(1)
        ...
        if ($candidates) {
            foreach ($candidates as $candidate) {
                ...
                foreach ($courses as $course) {
                    ...
                    if ($score_marker_1) {
                        $score_marker_2 = ScoreMarkerTwo::whereCandidateIndex(
                            ...
                            ->whereCourseKey($course->course_key)
                            ->whereExamId($score_marker_1->exam_id)
                            ->first();

                        $total_score_1 = $score_marker_1->total_score;
                        $total_score_2 = $score_marker_2 ? $score_marker_2->total_score : 0;

                        $course_average =
                            $total_score_2 > 0
                                ? ($total_score_2 + $total_score_1) / 2
                                : $total_score_1;
                ...
                ScoreMarkerOne::whereCandidateIndex($candidate->candidate_index)->update([
                    'new' => 0,
                ]);
            }
        }
